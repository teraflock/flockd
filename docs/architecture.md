...
| `internal/runtime/llamacpp` | Artifact fetcher (pinned manifest, SHA256-verify, cosign signature verification of the tarball and of `<manifest>.sig` against a pinned ECDSA P-256 key via the stdlib — `trust.go`; `runtime.artifact_signing_key` overrides the pin, `runtime.require_signature` refuses unsigned builds; the `.tarball.verified` stamp records which policy a cached extract was trusted under), supervisor (health-gate, crash-restart with backoff), and the HTTP/SSE translation to llama-server's OpenAI API on an ephemeral loopback port. |
...
| `internal/modelops` | On-demand model operations (catalog, downloads, load/unload/default) and the load path's memory admission: unload idle instances (mesh first, LRU, never the default or an in-flight model) to fit the budget, else `ErrOverMemory`; idle unload loop; measured footprints (host, plus `nvidia-smi` VRAM on discrete GPUs) for status and heartbeats. |
| `internal/assign` | Coordinator placement executor (plan 05): applies `ModelAssignment` pushes (`assign`, `stage`, `evict_model_ids`) under the operator's consent (`mesh_managed`, budget, pin/exclude), waits for AC power, reports `assigned`/`downloading`/`ready`/`cached`/`declined`/`failed`/`evicted` — each with `origin` operator\|mesh — over the tunnel and as `model_assignment` SSE events, and builds the Hello/heartbeat model list. `cached` = on disk, not loaded (memory budget full, idle-unloaded, or only staged); a placement is never declined for memory alone. |
